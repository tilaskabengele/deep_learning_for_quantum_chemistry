...
class SchNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        r_ij_1=r_ij.unsqueeze(-1).permute(0,3,1,2).cuda()

        conv2d1=nn.Conv2d(1,100,(1,num_at),stride=(1,1),padding=(1,1)).cuda()
        new_filter=conv2d1(r_ij_1).cuda()
        new_filter=new_filter.reshape(100,-1).cuda()
        Dense2d1=nn.Linear(new_filter.size()[1],num_at*(num_at-1)*20).cuda()
        sp1=nn.ReLU()
        new_filter=Dense2d1(new_filter)
