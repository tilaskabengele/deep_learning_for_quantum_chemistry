...
class SchNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        positions1=positions.view(100,-1)
        m=positions1.size()[1]
        dense1=nn.Linear(m,m*20).cuda()
        dense2=nn.Linear(m*20,num_at*(num_at-1)*20).cuda()
        sp1=nn.ReLU()
        sp2=nn.ReLU()
        new_filter=dense1(positions1)
        new_filter=sp1(new_filterb)
