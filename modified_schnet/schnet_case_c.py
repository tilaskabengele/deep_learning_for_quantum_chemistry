...
class SchNetInteraction(nn.Module):
    ...
    def __init__(
        ...
        self.conv1_network = nn.Sequential(
            nn.Conv2d(in_channels=3,out_channels=12,kernel_size=2,stride=1,padding=1),
            nn.BatchNorm2d(num_features=12),
            nn.Linear(in_features=12,out_features=30)
        )


    def forward(self, x, r_ij,v_ij, neighbors, neighbor_mask, f_ij=None):
        ...
        v = self.cfconv(x, r_ij,v_ij, neighbors, neighbor_mask, f_ij)
        v = self.dense(v)
        return v
...
class SchNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #Added part
        num_at=atomic_numbers.size()[1]
        conv1=nn.Conv1d(in_channels=3,out_channels=200,kernel_size=num_at,stride=1,padding=1).cuda()        
        
        sp1=nn.ReLU()
        sp2=nn.ReLU()
        
        new_filter= conv1(positions.transpose(1,2).cuda())
        new_filter=new_filter.view(100,-1)
        dense1=nn.Linear(new_filter.size()[1],2*400).cuda()
        new_filter=dense1(new_filter.cuda())
        new_filter=sp1(new_filter)
        dense2=nn.Linear(new_filter.size()[1],20*num_at*(num_at-1)).cuda()
        new_filter=dense2(new_filter)
        new_filter=sp2(new_filter)
        new_filter=new_filter.view(100,num_at,num_at-1,20)

        # compute interaction block to update atomic embeddings
        for interaction in self.interactions:
            v = interaction(x, r_ij, neighbors, neighbor_mask, f_ij=new_filter)
            x = x + v
            if self.return_intermediate:
                xs.append(x)
